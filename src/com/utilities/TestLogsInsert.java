...
//    DataBaseMySql dataBaseMySql;
    private TestLogsInsert() {
        createDBLogFile();
//        dataBaseMySql = new DataBaseMySql("jdbc:url:port?useSSL=false&useUnicode=yes&characterEncoding=UTF-8;appuser;appuser_pwd#1;");
    }
...
//        The Files.createDirectories() creates a new directory and parent directories that do not exist. This method does not throw an exception if the directory already exists.
        try {
            Files.createDirectories(Paths.get("./Reports/Reports_"));
            writer = new PrintWriter("./Reports/Reports_"+File.separator+"DBInsertQueries.txt", "UTF-8");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void copyFile(File source) throws IOException {
        Files.createDirectories(Paths.get("./Reports/Reports_"));
        File dest = new File("./Reports/Reports_"+File.separator+source.getName());
        try {
